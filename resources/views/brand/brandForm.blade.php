                                    ...
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="brandName" id="brandName" class="form-control" value="{{ isset($id) ? $brand['brandName'] : '' }}" placeholder="Enter Brand Name" required>
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label>Brand Icon (Recommended size: W: 186px x H: 74px)</label>
                                                <div id="imageUploadDropzone" class="dropzone"></div>
                                                @if (isset($id) && $brand['image'])
                                                    <div class="dropzone-preload col-sm-2 text-center">
                                                        <img src="{{ $brand['image'] }}" class="img-fluid">
                                                        <a href="javascript:void(0)" class="btn btn-xs btn-danger" onclick="deleteFile('{{ $brand['image'] }}')"><i class="fa fa-trash"></i></a>
                                                    </div>
                                                @endif
                                                <input type="hidden" name="image" id="image" class="form-control" value="{{ isset($id) ? $brand['image'] : '' }}" placeholder="Image">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
@section('scripts')
    ...
    <script>
        var submitUrl = "{{ route('brand.store') }}";
        var redirectUrl = "{{ route('brand.list') }}";
        var brand, brandId = "";
        @if (isset($id))
            var brand = @json($brand);
            var brandId = @json($id);
            submitUrl = "{{ route('brand.update', ['id' => ':id']) }}";
            submitUrl = submitUrl.replace(':id', brandId);
        @endif
